...
async def list_notes(
    ...
    doctor: User = Depends(require_roles(UserRole.DOCTOR, UserRole.ADMIN, UserRole.NURSE)),
) -> List[NoteOut]:
...
async def create_note(body: NoteCreate, doctor: User = Depends(require_roles(UserRole.DOCTOR))) -> NoteOut:
    note = MedicalNote(
        patient_id=body.patient_id,
        doctor_id=doctor.id,
...
async def list_referrals(doctor: User = Depends(require_roles(UserRole.DOCTOR))) -> List[ReferralOut]:
    items = await Referral.find(Referral.doctor_id == doctor.id).to_list()
    return [ReferralOut(_id=r.id, urgency=r.urgency.value, status=r.status.value, **r.model_dump(exclude={"id", "urgency", "status"})) for r in items]


@router.post("/referrals", response_model=ReferralOut, status_code=status.HTTP_201_CREATED)
async def create_referral(body: ReferralCreate, doctor: User = Depends(require_roles(UserRole.DOCTOR))) -> ReferralOut:
    from app.core.enums import ReferralUrgency
    ...
    await ref.insert()
    return ReferralOut(_id=ref.id, urgency=ref.urgency.value, status=ref.status.value, **ref.model_dump(exclude={"id", "urgency", "status"}))
...
async def list_family(patient: User = Depends(require_roles(UserRole.PATIENT))) -> List[FamilyMemberOut]:
...
async def add_family(body: FamilyMemberCreate, patient: User = Depends(require_roles(UserRole.PATIENT))) -> FamilyMemberOut:
...
async def remove_family(member_id: PydanticObjectId, patient: User = Depends(require_roles(UserRole.PATIENT))) -> MessageResponse:
...
async def get_insurance(patient: User = Depends(require_roles(UserRole.PATIENT))) -> Optional[InsuranceOut]:
...
async def get_my_availability(doctor: User = Depends(require_roles(UserRole.DOCTOR))) -> AvailabilityOut:
...
async def get_my_earnings(doctor: User = Depends(require_roles(UserRole.DOCTOR))) -> EarningsSummary:
...
async def get_health_timeline(patient: User = Depends(require_roles(UserRole.PATIENT))) -> List[TimelineEvent]:
