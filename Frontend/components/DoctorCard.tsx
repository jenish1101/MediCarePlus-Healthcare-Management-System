...
const DoctorCard: React.FC<DoctorCardProps> = ({ doctor, index = 0 }) => {
  ...
  return (
    <motion.article
      ...
      className="group flex h-full flex-col rounded-2xl border border-gray-200 bg-white shadow-sm transition-shadow hover:shadow-lg"
    >
      ...
      <div className="flex flex-1 flex-col gap-5 p-5 sm:p-6">
        ...
        <div className="flex items-start gap-4">
          <div className="shrink-0">
            ...
            <img
              src={doctor.avatar}
              alt={doctor.name}
              className="h-[88px] w-[88px] rounded-2xl border border-gray-200 bg-gray-50 object-cover object-center shadow-sm sm:h-24 sm:w-24"
            />
          </div>
          <div className="min-w-0 flex-1 pt-0.5">
            ...
            <h3 className="text-lg font-bold leading-tight text-gray-900 transition-colors group-hover:text-blue-600 sm:text-xl">
              {doctor.name}
            </h3>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <div className="rounded-xl border border-gray-100 bg-gray-50 px-3 py-2.5 text-center">
            ...
            <p className="mt-0.5 text-sm font-bold text-gray-900">{doctor.experience} yrs</p>
          </div>
          <div className="rounded-xl border border-gray-100 bg-gray-50 px-3 py-2.5 text-center">
            <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-400">Fee</p>
            <p className="mt-0.5 text-sm font-bold text-emerald-700">${doctor.fees}</p>
          </div>
          <div className="col-span-2 rounded-xl border border-gray-100 bg-gray-50 px-3 py-2.5 sm:col-span-2">
            <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-400">Location</p>
            <p className="mt-0.5 flex items-center justify-center gap-1 text-sm font-semibold text-gray-900 sm:justify-start">
        ...
        {/* Qualifications */}
        <div>
          <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-400">Qualifications</p>
        ...
        {/* Availability */}
        <div>
          <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-400">Available days</p>
          <div className="flex flex-wrap gap-1.5">
            {doctor.availability.map((day) => (
              <span
                key={day}
                className="rounded-lg border border-green-100 bg-green-50 px-2.5 py-1 text-xs font-medium text-green-700"
              >
                {day}
              </span>
            ))}
        ...
        <div className="mt-auto flex gap-3 border-t border-gray-100 pt-4">
          ...
          <button
            ...
            className="inline-flex items-center justify-center rounded-xl border-2 border-purple-200 px-4 py-3 text-purple-600 transition-colors hover:bg-purple-50"
