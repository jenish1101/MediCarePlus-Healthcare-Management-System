...
const Toggle: React.FC<{ on: boolean; onClick: () => void }> = ({ on, onClick }) => (
  <button
    type="button"
    onClick={onClick}
    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${on ? 'bg-purple-600' : 'bg-gray-300 dark:bg-gray-600'}`}
  >
      ...
      <DashboardLayout role="lab_tech">
        <div className="space-y-6 max-w-3xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="text-gray-600 dark:text-gray-400">Manage lab preferences and alerts</p>
          </motion.div>
          ...
          >
            ...
            <form className="grid sm:grid-cols-2 gap-6" onSubmit={(e) => e.preventDefault()}>
              <div className="sm:col-span-2">
                ...
                <input type="password" placeholder="••••••••" className="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent dark:bg-gray-900 dark:text-gray-100 dark:placeholder-gray-500" />
              </div>
              <div>
          ...
          >
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">Clear all completed test records. This action cannot be undone.</p>
            <button className="px-5 py-2.5 border border-red-600 dark:border-red-500 text-red-600 dark:text-red-400 rounded-lg font-medium hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
              Clear Records
            </button>
          </motion.div>
        </div>
